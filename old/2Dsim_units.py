...
class Printed_Matter:
    ...
    def __init__(self, strips_per_unit = 10, num_units = 20, rect_width = 0.1):
        self.dir = dir
        ...
        self.rectangle_centers = [1 + val*rect_width + rect_width/2 for val in range(strips_per_unit*num_units)]
    ...
    def take_photo(self, camera):
        ...
        for i in range(len(camera.pixel_centers)):
            ...
            for j in range(len(p.Repeated_H)):
                if camera.dir == "left":
                    j_ = j
                else:
                    j_ = len(p.Repeated_H) - j - 1
                
                rect_height = p.Repeated_H[j_]
                rect_center = p.rectangle_centers[j_]
                ...
                if self.intersect(ray, rectangle, np.array([cix, ciy]), dir):
                    ...
                    R[i] = p.COL[j_%p.strips_per_unit]
                    break
        camera.set_R(R)
        return R
...
def get_gradient():
    ...
    for i in range(len(p.H)):
        change = p.pertubate_height(i)
        update(cameras, p)
        loss = (camera0.get_cam_loss() + camera1.get_cam_loss())/2
        gradient[i] = loss-original_loss/change           #loss wrt change in height i
